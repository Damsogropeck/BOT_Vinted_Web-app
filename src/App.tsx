...
export default function App() {
  ...
  const openItemInNewTab = useCallback((itemUrl: string) => {
            ...
            <div className="flex flex-col gap-4">
              {filteredItems.map((item) => (
                ...
                >
                  ...
                  <div
                    className="flex-1 p-5 flex flex-col min-w-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-violet-500/60 focus:ring-offset-2 focus:ring-offset-[#050505]"
                    role="link"
                    tabIndex={0}
                    onClick={() => openItemInNewTab(item.itemUrl)}
                    onKeyDown={(event) => {
                      if (event.key === 'Enter' || event.key === ' ') {
                        event.preventDefault();
                        openItemInNewTab(item.itemUrl);
                      }
                    }}
                  >
